...
const AddTicket = ({ onClose, getTickets }) => {
    ...
    const Validation_Schema = Yup.object({
        ...
        // type: Yup.string('').required("مقدار این فیلد نمیتواند خالی باشد"),
        type: Yup.object().nullable().required('مقدار این فیلد نمیتواند خالی باشد'),
    });
    ...
    const handleSubmit = async (values, { resetForm }) => {
        ...
        if (values?.type) formData.append('type', values.type.id);
    ...
    return (
        <DDialogWrapper open onClose={onClose}>
            ...
            <Box display="grid" mt="30px" mb="0px" sx={{ borderTop: '1px solid #EEEEEE', paddingTop: '30px' }}>
                <Grid container spacing={2} sx={{ padding: '0px 10px 0 40px' }}>
                    <Grid item xs={12}>
                        <Formik initialValues={addFormInitialValues} validationSchema={Validation_Schema} onSubmit={handleSubmit}>
                                    ...
                                    <Grid columnSpacing={8} rowSpacing="14px" container>
                                        <Grid item xs={12} md={12}>
                                            <DSelect
                                                name="type"
                                                label={'ارسال به'}
                                                placeholder="دریافت کننده تیکت را انتخاب نمایید"
                                                formControlStyle={{ height: '42px', textAlign: 'right' }}
                                                defaultOptions={typeOption}
                                                onSelect={(newValue) => { 
                                        ...
                                        <Grid item xs={12} md={12}>
                                            ...
                                            <p style={textHelper}>
                                                پسوند های مجاز: .zip, .jpg, .jpeg, .png, .rar, .txt, .pdf (Max file size: 10MB)
                                            </p>
                                        </Grid>
