...
const Ticket = () => {
    ...
    const [breadCrumbLinks, setBreadCrumbLinks] = useState([
        ...
        { path: '/app/organization/management-tickets', title: 'پشتیبانی' },
        { path: `/app/organization/management-tickets/${ticketId}`, title: ` جزییات تیکت ${ticketId}#` },
    ]);
    ...
    const handleSubmit = async (values, { resetForm }) => {
        ...
        await OrganizationService.sendMessage(formData, ticket.id)
            .then((res) => {
                setSnackBarData({
                    show: true,
                    data: {
    ...
    const getTicket = () => {
        ...
        (async () => {
            await OrganizationService.get(`tickets/${ticketId}`, { signal })
                .then((res) => {
                    ...
                    <Grid item xs={4.5}>
                        <DBox sx={{ p: '20px 28px' }}>
                            ...
                            <Box
                                ...
                                <span>
                                    {ticket.created_at &&
                                        `${new Date(ticket.created_at).toLocaleTimeString('fa-IR')} - ${new Date(
                                            ticket.created_at
                            ...
                            <Box
                                ...
                                <span>
                                    {' '}
                                    {ticket.update_at
                                        ? `${new Date(ticket.update_at).toLocaleTimeString('fa-IR')} - ${new Date(
                                              ticket.update_at
                                          ).toLocaleDateString('fa-IR')}`
                                        : `${new Date(ticket.created_at).toLocaleTimeString('fa-IR')} - ${new Date(
                                              ticket.created_at
                    ...
                    <Grid item xs={7.5}>
                        <DBox sx={{ p: '20px 0' }}>
                            ...
                            <Box
                                ...
                                <Accordion
                                    sx={!expanded ? accordionStyle : accordionExpandedStyle}
                                    expanded={expanded}
                                    onChange={(e, expanded) => {
                                                    ...
                                                    <Grid columnSpacing={8} rowSpacing="14px" container>
                                                        ...
                                                        <Grid item xs={12} md={12}>
                                                            <CustomInputFile
                                                                ...
                                                                fileName={
                                                                    selectedPdf ? selectedPdf.name : 'فایلی انتخاب نشده است.'
                                                                }>
                                                                انتخاب فایل
                                                            </CustomInputFile>
                                                            <p style={textHelper}>
                                                                پسوند های مجاز: .zip, .jpg, .jpeg, .png, .rar, .txt, .pdf (Max
                                                                file size: 10MB)
                                                            </p>
                                                        </Grid>
                                ...
                                item.sender ? (
                                    <Box
                                        ...
                                        <Box
                                            ...
                                            <span>
                                                {item.created_at &&
                                                    `${new Date(item.created_at).toLocaleTimeString('fa-IR')} - ${new Date(
                                                        item.created_at
                                                    ).toLocaleDateString('fa-IR')}`}
                                            </span>
                                        </Box>
                            ...
                            <Link
                                style={{ textDecoration: 'none', width: '100%', display: 'grid' }}
                                to={`/app/organization/management-tickets`}>
                                <Box
                                    sx={{
                                        p: '6px 0',
                                        display: 'flex',
