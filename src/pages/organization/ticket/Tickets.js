...
import WalletRequestListItem from '../wallet/WalletRequestListItem';
...
import CreditRequest from '../wallet/CreditRequest';
...
const breadCrumbLinks = [{ path: '/app/organization/dashboard/', title: 'پیشخوان' }, { title: 'پشتیبانی' }];
...
const Tickets = () => {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            await OrganizationService.get(`tickets?${params.toString()}`)
                .then((res) => {
                    setTickets(res.data.data);
                    setTotalPage(res.data.meta.last_page);
                })
    ...
    const closeTicket = async (id) => {
        ...
        await OrganizationService.closeTicket(id)
            .then((res) => {
                setShowDeleteDialog(false);
                setFilters({ ...filters });
                setSnackBarData({
                            ...
                            <Formik initialValues={Initial_Values} onSubmit={handleFilterSubmit}>
                                <Form>
                                    <Box width="100%" mb="1.3rem">
                                        <CustomInputSearch sx={{ width: '100% !important' }} placeholder={'جستجو '} name="name" />
                                    </Box>
                                </Form>
                            </Formik>
                        </Grid>
                                    ...
                                    {tickets.length > 0 ? (
                                        tickets.map((ticket, index) => (
                                            <TicketListItem
                                                ...
                                                baseUrl="/app/organization/management-tickets/"
