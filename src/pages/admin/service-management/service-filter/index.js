...
import SvgExcel from "../../../../assets/icone/svg/Excel";
import '../../../../assets/style/scrollbar.css';
import SelectCompany from "../../../../components/new/pages/admin/serviceManagement/serviceFilter/SelectCompany";
import SelectCategories from "../../../../components/new/pages/admin/serviceManagement/serviceFilter/SelectCategories";
...
const breadCrumbLinks = [
    { link: '/app/admin', title: 'پیشخوان', dash: '/' },
    { link: '/app/admin/service-management', title: 'مدیریت خدمات', dash: '/' },
    { link: '/app/admin/service-management/service-filter', title: 'فیلتر خدمات' },
];
...
const ServiceFilterPage = () => {
    ...
    const [filters, setFilters] = useState({ page: 1, per_page: 100, type: 'BASIC' });
    const [filtersChanged, setFiltersChanged] = useState(false);
    const [totalPage, setTotalPage] = useState(1);

    const [selectedTableItems, setSelectedTableItems] = useState([]);
    const [selectedCompany, setSelectedCompany] = useState({});
    const [selectedCompanyChanged, setSelectedCompanyChanged] = useState(false);
    ...
    const handleCompanyCheckboxChange = (item) => {
        setSelectedCompany(item);
        setSelectedCompanyChanged(true);
    };
    ...
    const getCompanies = async () => {
        ...
        queryString.append('per_page', '1000');
        await adminService
            .getCompanies(queryString.toString())
            .then((res) => {
    ...
    const getServicesFilteredByCompany = async (type=filters.type) => {
        if(isSelectedCompanyEmpty()) return { reload: false, data: [] };
        const queryString = new URLSearchParams();
        queryString.append('page', filters.page);
        queryString.append('per_page', filters.per_page);
        queryString.append('type', type);
        queryString.append('company_id', selectedCompany.id);
        try {
            const res = await adminService.getServices(queryString.toString());
    ...
    const submitServices = async () => {
        ...
        let data = [];
        if(filters.type === 'BASIC') {
            data = await getServicesFilteredByCompany('COMPANY');
            if(data.reload)
                data = await getServicesFilteredByCompany('COMPANY');
        } else if(filters.type === 'COMPANY') {
            data = await getServicesFilteredByCompany('BASIC');
            if(data.reload)
                data = await getServicesFilteredByCompany('BASIC');
        }

        let tableItemIndex = 0;
        const queryString = new URLSearchParams();
        queryString.append('company_id', selectedCompany.id);
        selectedTableItems.forEach((service, index) => {
            queryString.append('service_ides[' + index + ']', service.id);
            tableItemIndex = index+1;
        });
        if(data.data) {
            data.data.forEach((service, index) => {
                queryString.append('service_ides[' + (tableItemIndex + index) + ']', service.id);
            });
        }
        await adminService
            ...
            .then((res) => {
                ...
                setSnackBarData({
                    show: true,
                    data: {
                        text: 'خدمات سازمان بروزرسانی شد.',
                        type: 'success',
                    },
                });
                reloadCompanyServices();
            })
            .catch((err) => {
                const errorMsg = err?.response?.data?.data && getErrorForSnackbar(err.response.data.data);
                errorMsg &&
    ...
    const isSelectedCompanyEmpty = () => {
        return Object.keys(selectedCompany).length === 0;
    }
    ...
    const appendCategoryServices = async () => {
        ...
        for (let service of categoryServices) {
            let found = companyServices.some(item => item.id === service.id);
    ...
    const reloadCategoryServices = async () => {
        ...
        if(isSelectedCompanyEmpty()) return [];
        setLoading({ ...loading, refresh: true });
    ...
    const reloadCompanyServices = async () => {
        if(isSelectedCompanyEmpty()) return[];
        setLoading({ ...loading, refresh: true });
        let data = await getServicesFilteredByCompany();
        if(data.reload)
            data = await getServicesFilteredByCompany();
        setCompanyServices(data.data);
    }

    const filterBasicServices = async () => {
        setFilters({...filters, type: 'BASIC'});
        setFiltersChanged(true);
    }
    const filterCompanyServices = async () => {
        setFilters({ ...filters, type: 'COMPANY' });
        setFiltersChanged(true);
    }
    useEffect(() => {
        getCompanies();
    ...
    useEffect(() => {
        if(selectedCompanyChanged) {
            setSelectedCompanyChanged(false);
            reloadCompanyServices();
        }
                            ...
                            <DLoadingWrapper loading={loading.initial}>
                                <>
                                    <Box style={{
                                        ...
                                        color: '#fff',
                                        fontSize: '16px'}}>
                                        <DTableHeader title={selectedCompany.name || '---'}>
                                            <LoadingButton
                                                variant="outlined"
                                    ...
                                    <Box style={{padding: '17px 30px 17px 30px', color: '#000', fontSize: '16px', background: '#fff'}}>
                                        <DTableHeader title="لیست خدمات در دسته بندی های انتخاب شده">
                                            <Button
                                                variant="contained"
                                                color="brandWarning"
                                                sx={{
                                                ...
                                                style={{
                                                    backgroundColor: filters.type === 'BASIC' ? '#0877BD' : '#F2F2F7',
                                                    color: filters.type === 'BASIC' ? 'white' : '#000',
                                                    opacity: filters.type === 'BASIC' ? '100%' : '40%',
                                                }}
                                                onClick={filterBasicServices}
                                                disabled={filters.type === 'BASIC'}>
                                                {'خدمات عمومی'}
                                            </Button>
                                            <Button
                                                variant="contained"
                                                ...
                                                style={{
                                                    backgroundColor: filters.type === 'COMPANY' ? '#0877BD' : '#F2F2F7',
                                                    color: filters.type === 'COMPANY' ? 'white' : '#000',
                                                    opacity: filters.type === 'COMPANY' ? '100%' : '40%',
                                                }}
                                                onClick={filterCompanyServices}
                                                ...
                                                <TableRow
                                                    ...
                                                    services.map((service, index) => (
                                                        <TableRow key={service.id} style={{
                                                            ...
                                                            <TableCell style={tableCellStyle}>{service.category || '---'}</TableCell>
                                                    ...
                                                    <DTableEmpty message={'هر دو فیلتر سازمان و دسته‌بندی را انتخاب کنید'}/>
                                                )
                                            )}
                                        </TableBody>
