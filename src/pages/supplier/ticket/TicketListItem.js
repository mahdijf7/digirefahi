...
const TicketListItem = (props) => {
    ...
    return (
        <TableRow style={props.style}>
            ...
            <TableCell style={tableCellStyle}>{status === 'OPEN' ? <span style={acceptStyle}>باز</span> :
                <span>بسته شده</span> || '---'}</TableCell>
            <TableCell
                style={tableCellStyle}>{update_at ? `${new Date(update_at).toLocaleTimeString('fa-IR')} - ${new Date(update_at).toLocaleDateString('fa-IR')}` : `${new Date(created_at).toLocaleTimeString('fa-IR')} - ${new Date(created_at).toLocaleDateString('fa-IR')}`}  </TableCell>
